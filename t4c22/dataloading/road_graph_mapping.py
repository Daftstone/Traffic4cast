...
class TorchRoadGraphMapping:
    def __init__(self, city: str, root: Path, df_filter, edge_attributes=None, skip_supersegments: bool = True,
        ...
        self.edge_index = torch.tensor(
            [[self.node_to_int_mapping[n] for n, _ in self.edges],
             [self.node_to_int_mapping[n] for _, n in self.edges]], dtype=torch.long
        )
            ...
            for i, edge in enumerate(self.edge_records):
                ...
                for j, attr in enumerate(edge_attributes):
                    if attr == "highway":
                        ...
                        self.edge_attr[i, j] = highway_dict[edge[attr]]
                    elif attr == "oneway":
                        if edge[attr] not in oneway_dict:
                            temp = len(oneway_dict)
                            oneway_dict[edge[attr]] = temp
                        self.edge_attr[i, j] = oneway_dict[edge[attr]]
                    else:
    ...
    def load_speeds_day_t(self, basedir: Path, city: str, split: str, day: str, t: int, idx: int) -> torch.Tensor:
    ...
    def load_inputs_day_t(self, basedir: Path, city: str, split: str, day: str, t: int, idx: int) -> torch.Tensor:
        ...
        x = torch.full(size=(len(self.counter_nodes) if self.counters_only else len(self.node_to_int_mapping), 4),
    ...
    def load_cc_labels_day_t(self, basedir: Path, city: str, split: str, day: str, t: int, idx: int) -> torch.Tensor:
        ...
        """
        df_y = load_cc_labels(basedir, city=city, split=split, day=day, with_edge_attributes=True,
